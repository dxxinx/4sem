...
namespace FlowerShop.Data
{
    public class EntityProductService
    {
        ...
        public async Task<Product> AddProductAsync(Product product)
        {
            using (var unitOfWork = new UnitOfWork())
            {
                var category = await GetOrCreateCategoryAsync(unitOfWork, product.Category);
                var manufacturer = await GetOrCreateManufacturerAsync(unitOfWork, product.Manufacturer);
        ...
        public async Task UpdateProductAsync(Product product)
        {
            using (var unitOfWork = new UnitOfWork())
            {
                var stored = await FindProductAsync(unitOfWork, product);
                if (stored == null)
                ...
                stored.CategoryEntity = await GetOrCreateCategoryAsync(unitOfWork, product.Category);
                stored.ManufacturerEntity = await GetOrCreateManufacturerAsync(unitOfWork, product.Manufacturer);
        ...
        public async Task DeleteProductAsync(Product product)
        {
            using (var unitOfWork = new UnitOfWork())
            {
                ...
                unitOfWork.Products.Remove(stored);
                await unitOfWork.SaveChangesAsync();
            }
        }
        ...
        private static async Task<Category> GetOrCreateCategoryAsync(IUnitOfWork unitOfWork, string name)
        {
            name = string.IsNullOrWhiteSpace(name) ? "Разное" : name.Trim();
            var category = await unitOfWork.Categories.Query().FirstOrDefaultAsync(c => c.Name == name);
            if (category != null)
                return category;

            category = new Category { Name = name, Description = "Добавлено через EF Code First" };
            unitOfWork.Categories.Add(category);
            return category;
        }

        private static async Task<Manufacturer> GetOrCreateManufacturerAsync(IUnitOfWork unitOfWork, string name)
        {
            name = string.IsNullOrWhiteSpace(name) ? "Новый поставщик" : name.Trim();
            var manufacturer = await unitOfWork.Manufacturers.Query().FirstOrDefaultAsync(m => m.Name == name);
            if (manufacturer != null)
                return manufacturer;

            manufacturer = new Manufacturer { Name = name, Country = "Беларусь", Phone = "+375" };
            unitOfWork.Manufacturers.Add(manufacturer);
            return manufacturer;
        }

        private static Task<Product> FindProductAsync(IUnitOfWork unitOfWork, Product product)
        {
            return product.ProductId > 0
                ? unitOfWork.Products.Query().FirstOrDefaultAsync(p => p.ProductId == product.ProductId)
                : unitOfWork.Products.Query().FirstOrDefaultAsync(p => p.Id == product.Id);
        }
    }
}
