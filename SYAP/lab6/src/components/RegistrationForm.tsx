...
export const RegistrationForm = () => {
  ...
  const validateStep = () => {
    ...
    if (state.currentStep === 1) result = Step1Schema.safeParse(state.formData);
    if (state.currentStep === 2) result = Step2Schema.safeParse(state.formData);
    if (state.currentStep === 3) result = Step3Schema.safeParse(state.formData);
  ...
  const submit = () => {
    ...
    setTimeout(() => {
      console.log("Данные формы:", state.formData);
  ...
  return (
    <div className="form-container">
      ...
      <div className="buttons">
        {state.currentStep > 1 && (
          <button onClick={() => dispatch({ type: "PREV_STEP" })}>Назад</button>
        )}

        {state.currentStep < 3 && <button onClick={next}>Далее</button>}
